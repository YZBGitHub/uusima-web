...
export default function App() {
  const [activeView, setActiveView] = useState<'home' | 'login' | 'registration' | 'certification' | 'config' | 'personal' | 'forgot-password' | 'profile' | 'school' | 'tenant' | 'personal-tenant' | 'join-org' | 'invite-landing' | 'invite-records'>('login');
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-800">
      ...
      <div className="fixed left-0 bottom-4 z-[100] flex items-end drop-shadow-xl">
        <div className={`bg-white border-y border-r border-slate-200 rounded-tr-xl rounded-br-xl transition-all duration-300 overflow-hidden flex flex-col shadow-inner ${isNavOpen ? 'w-48 opacity-100' : 'w-0 opacity-0 pointer-events-none'}`}>
          <div className="py-3 px-2 flex justify-between items-center border-b border-slate-50 shadow-sm">
            <span className="text-xs font-bold text-slate-400 px-2 uppercase tracking-wider">页面切换</span>
          ...
          <div className="py-2 px-2 max-h-[70vh] overflow-y-auto no-scrollbar space-y-0.5">
            {navItems.map(item => (
              <button 
                key={item.id} 
                onClick={() => { setActiveView(item.id as any); setIsNavOpen(false); }} 
      ...
      <div className="flex-1 relative overflow-x-hidden">
        {activeView === 'home' && <Home onNavigate={(view) => setActiveView(view)} />}
        {activeView === 'login' && <Login onNavigate={(view) => setActiveView(view)} />}
        {activeView === 'registration' && <Registration onNavigate={(view) => setActiveView(view)} />}
        {activeView === 'config' && <ConfigManagement onNavigate={(view) => setActiveView(view as any)} />}
        {activeView === 'personal' && <Personal onNavigate={(view) => setActiveView(view as any)} />}
        {activeView === 'forgot-password' && <ForgotPassword onNavigate={(view) => setActiveView(view)} />}
        ...
        {activeView === 'invite-landing' && <InviteLanding onNavigate={(view) => setActiveView(view)} />}
