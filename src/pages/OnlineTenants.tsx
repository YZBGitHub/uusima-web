...
import { Search, Edit2, Trash2, Shield, ShoppingCart, Users, Activity, X, MoreHorizontal, Info , FileText} from 'lucide-react';
...
export default function OnlineTenants() {
  ...
  const [modalState, setModalState] = useState<{
    isOpen: boolean;
    type: 'resource' | 'orders' | 'organization' | 'members' | null;
    tenant: Tenant | null;
  }>({
    isOpen: false,
    type: null,
  ...
  const openModal = (type: 'resource' | 'orders' | 'organization' | 'members', tenant: Tenant) => {
  ...
  const renderModalContent = () => {
    ...
    switch (modalState.type) {
      case 'resource':
        content = <TenantResourceOverview tenantType="online" />;
        break;
      case 'orders':
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-white border-b border-slate-200 shrink-0">
          <h3 className="text-lg font-semibold text-slate-800">
            {modalState.tenant.name} - {
              ...
              modalState.type === 'organization' ? '组织管理' :
              '成员管理'
            }
          </h3>
              ...
              {currentTenants.map((tenant) => (
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end space-x-3">
                      ...
                      <button 
                        onClick={() => openModal('organization', tenant)}
                        className="text-[#108ee9] hover:text-blue-700 font-medium text-xs transition-colors"
                      >
                        组织管理
                      </button>
                      <button 
                        onClick={() => openModal('members', tenant)}
                        className="text-[#108ee9] hover:text-blue-700 font-medium text-xs transition-colors"
                      >
                        成员管理
                      </button>
