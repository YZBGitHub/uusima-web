...
import { Bell, UserCircle, Settings, ChevronDown, Check, LogOut, CheckCircle2, XCircle, Gem, X, Edit, Shield, BadgeCheck, Activity, Clock, ArrowLeftRight, User, IdCard, Package, Gift, Search, CalendarDays, ChevronUp, ChevronRight, ChevronLeft, Calendar, FileText, LayoutGrid, RefreshCw } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { PieChart, Pie, Cell, ResponsiveContainer, Tooltip as RechartsTooltip } from 'recharts';
import Certification from './Certification';

const MOCK_USAGE_RECORDS = [
  { id: 1, startTime: '2026-06-11 09:58:22', account: '18033926960', item: 'AI技能分析系统-AI综合画像', usage: '19800 Token(s)', type: 'token', amount: 19800 },
  { id: 2, startTime: '2026-06-10 17:25:05', account: '15396005420', item: '虚拟仿真实验助手', usage: '45 分钟', type: 'duration', amount: 45 },
  { id: 3, startTime: '2026-06-10 15:27:36', account: '18033926960', item: 'AI技能分析系统-生成工具', usage: '543 Token(s)', type: 'token', amount: 543 },
  { id: 4, startTime: '2026-06-10 13:45:46', account: '13805009731', item: '通用助手', usage: '335 Token(s)', type: 'token', amount: 335 },
  { id: 5, startTime: '2026-06-10 13:45:43', account: '13805009731', item: '电子电路虚拟仿真实验', usage: '30 分钟', type: 'duration', amount: 30 },
  { id: 6, startTime: '2026-06-09 16:56:43', account: '13023112851', item: 'AI生成PPT', usage: '2 次', type: 'ppt', amount: 2 },
  { id: 7, startTime: '2026-06-09 15:24:48', account: '13023112851', item: '深度学习模型训练虚拟仿真', usage: '120 分钟', type: 'duration', amount: 120 },
  { id: 8, startTime: '2026-06-09 14:43:15', account: '13023112851', item: 'AI生成PPT', usage: '1 次', type: 'ppt', amount: 1 },
  { id: 9, startTime: '2026-06-09 14:42:53', account: '13023112851', item: '人工智能前端设备应用实训平台智能体', usage: '7110 Token(s)', type: 'token', amount: 7110 },
  { id: 10, startTime: '2026-06-09 14:41:59', account: '13023112851', item: '人工智能前端设备应用实训平台智能体', usage: '7296 Token(s)', type: 'token', amount: 7296 }
];

export default function Personal({ onNavigate }: { onNavigate?: (view: string) => void } = {}) {
  const [isTenantOpen, setIsTenantOpen] = useState(false);
  const [isUserMenuOpen, setIsUserMenuOpen] = useState(false);
  const [activeMenu, setActiveMenu] = useState<'info' | 'certification' | 'messages' | 'assets' | 'security' | 'advanced_security'>('info');
  const [activeMessageTab, setActiveMessageTab] = useState<'todo' | 'invites' | 'system' | 'audit'>('todo');
  ...
  const [todoCategory, setTodoCategory] = useState<'pending' | 'approved' | 'rejected'>('pending');
  ...
  const [orgRecordsTab, setOrgRecordsTab] = useState<'all' | 'pending' | 'approved' | 'rejected'>('all');
  ...
  const handleExitOrg = () => {
    if (orgToExit) {
      setProfileData(prev => ({
        ...prev,
        organizations: prev.organizations.filter(org => org.name !== orgToExit)
      }));
  ...
  const [adminApprovalFilter, setAdminApprovalFilter] = useState<'school' | 'enterprise'>('school');
  const [adminApprovalDetailModal, setAdminApprovalDetailModal] = useState<any | null>(null);
  ...
  const [verifyLoginNameStatus, setVerifyLoginNameStatus] = useState<'success' | 'error' | 'idle'>('idle');
  ...
  const [usageTypeFilter, setUsageTypeFilter] = useState<'all' | 'token' | 'duration' | 'ppt'>('all');
  const [usageSortOrder, setUsageSortOrder] = useState<'desc' | 'asc' | 'none'>('none');
  ...
  const signedInDates = ['2026-06-01', '2026-06-05', '2026-06-08', '2026-06-10', '2026-06-11'];
  
  const getFilteredAndSortedRecords = () => {
    let result = [...MOCK_USAGE_RECORDS];
    if (usageSearchQuery) {
      result = result.filter(r => r.startTime.includes(usageSearchQuery));
    }
    if (usageItemSearch) {
      result = result.filter(r => r.item.includes(usageItemSearch));
    }
    if (usageDateFilter) {
      result = result.filter(r => r.startTime.startsWith(usageDateFilter));
    }
    if (usageTypeFilter !== 'all') {
      result = result.filter(r => r.type === usageTypeFilter);
    }
    if (usageSortOrder !== 'none') {
      result.sort((a, b) => {
        if (usageSortOrder === 'desc') return b.amount - a.amount;
  ...
  const todos = [
    { 
      id: 1, type: '教师认证', template: 'cert', name: '张三', role: '教师', dept: '计算机科学学院',
      avatar: '张', status: 'pending', 
      details: [
        { label: '姓名', value: '张三' },
    ...
    { 
      id: 2, type: '学生认证', template: 'cert', name: '李四', role: '学生', dept: '软件工程系',
      avatar: '李', status: 'pending',
      details: [
        { label: '姓名', value: '李四' },
    ...
    { 
      id: 3, type: '加入申请', template: 'link', name: '王五', role: '企业员工', dept: '研发中心二部',
      avatar: '王', status: 'approved',
      details: [
        { label: '姓名', value: '王五' },
        { label: '邀请人', value: '李总' },
    ...
    { 
      id: 4, type: '教师认证', template: 'cert', name: '陈六', role: '教师', dept: '物理学院',
      avatar: '陈', status: 'rejected',
      rejectReason: '提交的证明文件不足，请提供手持工牌照片或加盖公章的在职证明。',
      details: [
        { label: '姓名', value: '陈六' },
  ...
  const notifications = [
    ...
    { id: 2, type: 'reject', message: '您的企业员工申请被驳回。原因：职位信息与实际不符' },
    { id: 3, type: 'asset', message: 'UUSIMA 游客体验套餐已下发（100w Token, 100分钟）' },
    { id: 4, type: 'success', message: '您提交的学生角色认证申请已通过' },
    { id: 5, type: 'reject', message: '您加入【复旦大学 - 计算机系】的申请被驳回。原因：未在新生名单中找到您的信息' },
  ...
  const [adminApprovals, setAdminApprovals] = useState([
    ...
    },
    {
      ...
      details: [
        ...
        { label: '凭证记录', value: '教师资格证复印件.png', isAttachment: true },
      ...
      <header className="h-14 bg-white border-b border-slate-200 flex items-center justify-between px-6 z-10 shrink-0">
        <div className="flex items-center space-x-6">
            ...
            <div className="flex items-center space-x-2">
              <span className="text-lg font-bold text-slate-800 tracking-tight">个人中心</span>
            </div>
        </div>
        <div className="flex items-center space-x-6">
          <button 
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className="w-56 bg-slate-50 border-r border-slate-200 flex flex-col py-4 shrink-0">
          <nav className="flex-1 space-y-2">
             <div>
               <div className="px-8 py-2 text-xs font-semibold text-slate-400 uppercase tracking-wider">账户设置</div>
               <div className="mt-1 space-y-1">
                 <button onClick={() => setActiveMenu('info')} className={`w-full flex items-center px-8 py-2.5 text-sm transition-colors ${activeMenu === 'info' ? 'bg-blue-50 text-blue-600 font-medium' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}`}>
                    <User className={`w-4 h-4 mr-3 ${activeMenu === 'info' ? 'text-blue-600' : 'text-slate-500'}`} /> 个人信息
                 </button>
                 <button onClick={() => setActiveMenu('certification')} className={`w-full flex items-center px-8 py-2.5 text-sm transition-colors ${activeMenu === 'certification' ? 'bg-blue-50 text-blue-600 font-medium' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}`}>
                    <IdCard className={`w-4 h-4 mr-3 ${activeMenu === 'certification' ? 'text-blue-600' : 'text-slate-500'}`} /> 身份认证
                 </button>
                 <button onClick={() => setActiveMenu('advanced_security')} className={`w-full flex items-center px-8 py-2.5 text-sm transition-colors ${activeMenu === 'advanced_security' ? 'bg-blue-50 text-blue-600 font-medium' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}`}>
                    <Shield className={`w-4 h-4 mr-3 ${activeMenu === 'advanced_security' ? 'text-blue-600' : 'text-slate-500'}`} /> 安全认证
                 </button>
               </div>
             </div>
             <div className="pt-2">
               ...
               <div className="mt-1 space-y-1">
                 <button onClick={() => setActiveMenu('assets')} className={`w-full flex items-center px-8 py-2.5 text-sm transition-colors ${activeMenu === 'assets' ? 'bg-blue-50 text-blue-600 font-medium' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}`}>
                    <Package className={`w-4 h-4 mr-3 ${activeMenu === 'assets' ? 'text-blue-600' : 'text-slate-500'}`} /> 我的资源
                 </button>
               </div>
             </div>
          </nav>
           ...
           {activeMenu === 'info' && (
             <div className="p-8 flex flex-col space-y-6 relative">
               ...
               <div className="flex flex-row pt-8">
                 ...
                 <div className="mb-6">
                   <label className="block text-sm font-medium text-slate-500 mb-2">账号：</label>
                   <div className="text-slate-900 text-sm py-1">{profileData.account}</div>
                 </div>
                 <div className="mb-6">
                   <label className="block text-sm font-medium text-slate-500 mb-2">
                     {isEditingProfile && <span className="text-red-500 font-bold mr-1">*</span>}用户名：
                   </label>
                   {isEditingProfile ? (
                     <input 
                       type="text" 
                     ...
                     <div className="text-slate-900 text-sm py-1">{profileData.userName}</div>
                   )}
                 </div>
                 <div className="mb-6">
                   <label className="block text-sm font-medium text-slate-500 mb-2">手机号：</label>
                   {isEditingProfile ? (
                     <input 
                       type="text" 
                     ...
                     <div className="text-slate-900 text-sm py-1">{profileData.phone}</div>
                   )}
                 </div>
                 <div className="mb-8">
                   <label className="block text-sm font-medium text-slate-500 mb-2">
                     {isEditingProfile && <span className="text-red-500 font-bold mr-1">*</span>}性别：
                   </label>
                   {isEditingProfile ? (
                     <div className="flex items-center space-x-6 h-10">
                       <label className="flex items-center cursor-pointer group">
                         ...
                         />
                         <span className="ml-2 text-sm text-slate-700 group-hover:text-slate-900">女</span>
                       </label>
                       <label className="flex items-center cursor-pointer group">
                         <input 
                           type="radio" 
                           ...
                           className="w-4 h-4 text-blue-500 bg-slate-50 border-slate-300 focus:ring-blue-500 focus:ring-offset-0" 
                         />
                         <span className="ml-2 text-sm text-slate-700 group-hover:text-slate-900">男</span>
                       </label>
                     </div>
                   ) : (
                     <div className="text-slate-900 text-sm py-1">{profileData.gender === 'female' ? '女' : '男'}</div>
                   )}
                 </div>
                 ...
                 {isEditingProfile && (
                   <div className="flex space-x-3">
                     ...
                     <button 
                       onClick={() => setConfirmConfig({ isOpen: true, title: '确认保存', content: '确定要保存修改后的个人信息吗？', onConfirm: () => {
               ...
               <div className="w-64 pl-16">
                 <label className="block text-sm font-medium text-slate-500 mb-4">头像</label>
                 <div className="w-24 h-24 border border-dashed border-slate-300 bg-slate-50/50 flex items-center justify-center cursor-pointer hover:bg-slate-100 transition-colors">
                   {isEditingProfile ? (
                     <span className="text-slate-400 text-3xl font-light leading-none mb-1">+</span>
                   ) : (
             ...
             <div className="mt-8 border-t border-slate-100 flex flex-col md:flex-row gap-8 pt-8">
               <div className="flex-1 bg-slate-50 border border-slate-200 rounded-xl p-6">
                 <div className="flex items-center justify-between mb-6">
                   ...
                   <button 
                     onClick={() => setProfileData(prev => ({ ...prev, isCertified: !prev.isCertified }))}
                 ...
                 {profileData.isCertified ? (
                   <div className="space-y-4">
                     <div className="flex items-center text-sm">
                       ...
                       <span className="text-slate-900 font-medium">张三</span>
                     </div>
                     <div className="flex items-center text-sm">
                       <span className="text-slate-500 w-24">认证类型：</span>
                       <span className="px-2.5 py-1 bg-blue-100 text-blue-700 rounded text-xs font-medium border border-blue-200">教师认证</span>
                     </div>
                     <div className="flex items-center text-sm">
                       <span className="text-slate-500 w-24">所属学校：</span>
                       <span className="text-slate-900 font-medium">某某大学</span>
                     </div>
                     <div className="flex items-center text-sm">
                       <span className="text-slate-500 w-24">工号/学号：</span>
                       <span className="text-slate-900 font-medium cursor-pointer" title="点击查看详情">20010******</span>
                     </div>
                   </div>
                 ) : (
                   <div className="flex flex-col items-center justify-center py-8 bg-white border border-slate-200 border-dashed rounded-lg shadow-sm">
                     ...
                     <div className="text-slate-500 text-sm mb-4">暂未完成身份认证</div>
                     <button 
               ...
               <div className="flex-1 bg-slate-50 border border-slate-200 rounded-xl p-6">
                 <div className="flex items-center justify-between mb-6">
                   <h3 className="text-base font-bold text-slate-800 flex items-center">
                     <svg viewBox="0 0 24 24" fill="none" className="w-5 h-5 mr-2 text-indigo-500" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
                     加入的组织
                   </h3>
                   <button 
                 ...
                 {profileData.organizations && profileData.organizations.length > 0 ? (
                   <div className="space-y-3">
                     {profileData.organizations.map((org: any, i: number) => (
                       <div key={i} className="flex items-center bg-white p-3 rounded-lg border border-slate-200 shadow-sm transition-shadow hover:shadow-md">
                         ...
                         <div className="flex-1 overflow-hidden">
                           <div className="text-sm font-medium text-slate-900 truncate mb-0.5">{org.name}</div>
                           <div className="flex items-center text-xs text-slate-500">
                             {org.status === 'pending' ? (
                               <>
                                 <span className="inline-block w-2 h-2 rounded-full bg-amber-400 mr-1.5"></span>
                                 <span className="text-amber-600 mr-2 font-medium">审核中</span>
                               </>
                             ) : (
                   ...
                   <div className="flex flex-col items-center justify-center py-10 bg-white border border-slate-200 border-dashed rounded-lg shadow-sm">
                     <div className="w-12 h-12 bg-slate-50 rounded-full flex items-center justify-center mb-3">
                       <svg viewBox="0 0 24 24" fill="none" className="w-6 h-6 text-slate-300" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
                     </div>
                     <div className="text-slate-400 text-sm">暂未加入其他组织</div>
                   </div>
                 )}
               </div>
             </div>
               ...
               <div className="max-w-5xl mx-auto">
                 <div className="mb-6">
                   ...
                   <div className="flex space-x-2 border-b border-slate-200 mt-4">
                      ...
                      >
                        待办事项 (待审批)
                        <span className="ml-1.5 min-w-[1.25rem] text-center text-xs bg-red-500 text-white px-1.5 py-0.5 rounded-full font-bold leading-none">12</span>
                      </button>
                      <button 
                      ...
                      >
                        <span>身份认证审核</span>
                        {adminApprovals.filter(a => a.status === 'pending').length > 0 && (
                    ...
                    {activeMessageTab === 'todo' && (
                      <div className="flex flex-col">
                        <div className="flex items-center justify-between p-4 border-b border-slate-100 bg-slate-50/50">
                          <div className="flex space-x-2">
                            <button onClick={() => {setTodoCategory('pending'); setSelectedTodos([]); setExpandedTodo(null);}} className={`px-4 py-1.5 text-sm font-medium rounded-full transition-colors ${todoCategory === 'pending' ? 'bg-blue-100 text-blue-700' : 'text-slate-600 hover:bg-slate-200'}`}>待审批</button>
                            <button onClick={() => {setTodoCategory('approved'); setSelectedTodos([]); setExpandedTodo(null);}} className={`px-4 py-1.5 text-sm font-medium rounded-full transition-colors ${todoCategory === 'approved' ? 'bg-green-100 text-green-700' : 'text-slate-600 hover:bg-slate-200'}`}>已通过</button>
                            <button onClick={() => {setTodoCategory('rejected'); setSelectedTodos([]); setExpandedTodo(null);}} className={`px-4 py-1.5 text-sm font-medium rounded-full transition-colors ${todoCategory === 'rejected' ? 'bg-red-100 text-red-700' : 'text-slate-600 hover:bg-slate-200'}`}>已驳回</button>
                          </div>
                          {todoCategory === 'pending' && (
                            <div className="flex items-center space-x-4">
                              ...
                              <span className="text-sm text-slate-500">已选 {selectedTodos.length} 项</span>
                              <button disabled={selectedTodos.length === 0} className="px-3 py-1.5 text-sm font-medium text-white bg-blue-600 rounded disabled:bg-blue-300 transition-colors">批量通过</button>
                            </div>
                          )}
                        </div>
                        <div className="divide-y divide-slate-100">
                          {filteredTodos.length === 0 ? (
                            ...
                            filteredTodos.map((todo) => (
                              <div key={todo.id} className="flex flex-col transition-colors hover:bg-slate-50/50">
                                <div className="flex items-center p-5 cursor-pointer" onClick={() => setExpandedTodo(expandedTodo === todo.id ? null : todo.id)}>
                                  {todoCategory === 'pending' && (
                                  ...
                                  <div className="flex items-center flex-1 min-w-0">
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <div className="flex items-center space-x-2">
                                        <span className="font-bold text-slate-900 truncate">{todo.name}</span>
                                        <span className="px-2 py-0.5 bg-slate-100 text-slate-600 text-xs rounded truncate max-w-[120px]">{todo.role}</span>
                                        {todoCategory === 'pending' && <span className="px-2 py-0.5 bg-amber-50 text-amber-600 text-xs rounded border border-amber-200">待审批</span>}
                                        {todoCategory === 'approved' && <span className="px-2 py-0.5 bg-green-50 text-green-600 text-xs rounded border border-green-200">已通过</span>}
                                        {todoCategory === 'rejected' && <span className="px-2 py-0.5 bg-red-50 text-red-600 text-xs rounded border border-red-200">已驳回</span>}
                                      </div>
                                      <p className="text-sm text-slate-500 mt-1 truncate">申请角色：{todo.role} | 部门：{todo.dept} | 类型：{todo.type}</p>
                                    </div>
                                  </div>
                                  <div className="flex items-center space-x-3 ml-4" onClick={(e) => e.stopPropagation()}>
                                    {todoCategory === 'pending' && (
                                      <>
                                        ...
                                        <button 
                                          onClick={() => setConfirmConfig({ isOpen: true, title: '确认通过', content: '确定要通过该认证申请吗？', onConfirm: () => {} })}
                                    ...
                                    <button className="text-slate-400 hover:text-slate-600 p-1">
                                      <ChevronDown className={`w-5 h-5 transition-transform ${expandedTodo === todo.id ? 'rotate-180' : ''}`} />
                                    </button>
                                  </div>
                                </div>
                                <AnimatePresence>
                                  {expandedTodo === todo.id && (
                                    ...
                                    >
                                      <div className="p-6">
                                        <h4 className="text-sm font-semibold text-slate-800 mb-4">表单详情</h4>
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-8">
                                          ...
                                          {todo.status === 'rejected' && todo.rejectReason && (
                                            <div className="flex flex-col space-y-1 sm:col-span-2">
                                              <span className="text-xs font-medium text-red-500">驳回原因</span>
                                              <span className="text-sm text-slate-900 bg-red-50 p-2 rounded border border-red-100">{todo.rejectReason}</span>
                                            </div>
                                          )}
                                        </div>
                                      </div>
                    ...
                    {activeMessageTab === 'system' && (
                      <div className="divide-y divide-slate-100">
                        {notifications.map((note) => (
                          <div key={note.id} className="flex items-center p-5 hover:bg-slate-50/50 transition-colors">
                            <div className="shrink-0 mr-4">
                              {note.type === 'success' && <CheckCircle2 className="w-5 h-5 text-green-500" />}
                              {note.type === 'reject' && <XCircle className="w-5 h-5 text-red-500" />}
                              {note.type === 'asset' && <Gem className="w-5 h-5 text-blue-500" />}
                            </div>
                    ...
                    {activeMessageTab === 'audit' && (
                      <div className="divide-y divide-slate-100">
                        <div className="p-4 bg-slate-50 flex items-center space-x-2 border-b border-slate-100">
                          <button onClick={() => setAdminApprovalFilter('school')} className={`px-4 py-1.5 text-sm font-medium rounded-full transition-colors ${adminApprovalFilter === 'school' ? 'bg-blue-100 text-blue-700' : 'text-slate-600 hover:bg-slate-200'}`}>院校认证</button>
                          <button onClick={() => setAdminApprovalFilter('enterprise')} className={`px-4 py-1.5 text-sm font-medium rounded-full transition-colors ${adminApprovalFilter === 'enterprise' ? 'bg-orange-100 text-orange-700' : 'text-slate-600 hover:bg-slate-200'}`}>企业认证</button>
                        </div>
                        {adminApprovals.filter(a => a.type === adminApprovalFilter).map((approval) => (
                          <div key={`approval-${approval.id}`} className="flex flex-col p-5 hover:bg-slate-50/50 transition-colors">
                            <div className="flex items-start justify-between">
                              <div>
                                <div className="flex items-center space-x-2 mb-2">
                                  <span className={`px-2 py-0.5 rounded text-xs font-medium ${approval.type === 'school' ? 'bg-blue-50 text-blue-700 border border-blue-200' : 'bg-orange-50 text-orange-700 border border-orange-200'}`}>
                                    {approval.type === 'school' ? '院校认证' : '企业认证'}
                                  </span>
                                  <h3 className="font-bold text-slate-800">{approval.organization}</h3>
                                </div>
                                <p className="text-sm text-slate-600">
                                  申请人：{approval.name} <span className="mx-2 text-slate-300">|</span> 申请身份：{approval.role}
                                </p>
                              ...
                              <div className="flex flex-col items-end">
                                {approval.status === 'pending' && (
                                  <button 
                                    onClick={() => setAdminApprovalDetailModal(approval)}
                                ...
                                {approval.status === 'approved' && <span className="text-sm font-medium text-green-600 flex items-center px-3 py-1.5 bg-green-50 rounded-md border border-green-100"><CheckCircle2 className="w-4 h-4 mr-1.5"/>已通过</span>}
                                {approval.status === 'rejected' && <span className="text-sm font-medium text-red-600 flex items-center px-3 py-1.5 bg-red-50 rounded-md border border-red-100" title={approval.rejectReason}><XCircle className="w-4 h-4 mr-1.5"/>已驳回</span>}
                              </div>
                            </div>
                          </div>
                        ))}
                        {adminApprovals.filter(a => a.type === adminApprovalFilter).length === 0 && (
...
{/* 驳回原因填写弹窗 (Modal) */}
                       ...
                       <div className="fixed inset-0 z-[101] flex items-center justify-center p-4 pointer-events-none">
                         ...
                         >
                            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                              <h2 className="text-lg font-semibold text-slate-900">填写驳回原因</h2>
                              <button 
                                onClick={() => setRejectModalOpen(false)}
                            ...
                            <div className="p-6">
                               <label className="flex items-center justify-between text-sm font-medium text-slate-700 mb-2">
                                 <span>驳回原因 <span className="text-red-500">*</span></span>
                                 <span className={`text-xs ${rejectReason.length < 50 || rejectReason.length > 200 ? 'text-red-500' : 'text-slate-500'}`}>
                                   {rejectReason.length} / 200 (需50-200字)
                                 </span>
                               </label>
                               <div className="flex flex-wrap gap-2 mb-3">
                                 <button onClick={() => setRejectReason('您提交的申请信息不完整，缺少平台规定的必要证明材料。请您仔细核对身份认证要求，补充所需的详细信息及证明文件后再次提交审核。')} className="px-3 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs rounded transition-colors border border-slate-200">信息不完整</button>
                                 <button onClick={() => setRejectReason('系统检测到您的工号/学号验证失败，可能是由于输入有误或对应机构在库名单中未包含该信息。请确认正确的学工号后重新发起申请。')} className="px-3 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs rounded transition-colors border border-slate-200">学工号验证失败</button>
                                 <button onClick={() => setRejectReason('由于您非本机构（学校/企业）的正式注册人员，暂不符合此身份角色的认证要求。如有特殊情况，请联系机构管理人员确认后重新申请。')} className="px-3 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs rounded transition-colors border border-slate-200">非本机构人员</button>
                               </div>
                               <textarea 
                                 value={rejectReason}
                            ...
                            <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex justify-end space-x-3">
                               ...
                               <button 
                                 disabled={rejectReason.length < 50 || rejectReason.length > 200}
                                 onClick={() => {
           ...
           {activeMenu === 'assets' && (
              <div className="p-8">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h1 className="text-2xl font-bold text-slate-800">我的资源</h1>
                    {activeTenantId === 'personal' && (
                      <div className="mt-2 text-sm text-slate-600 bg-blue-50 px-3 py-1 inline-flex items-center rounded-full border border-blue-100">
                        <BadgeCheck className="w-4 h-4 mr-1.5 text-blue-600" />
                        当前套餐: <span className="font-semibold text-blue-700 ml-1">免费体验套餐</span>
                      </div>
                    )}
                  </div>
                  {activeTenantId === 'personal' && (
                        ...
                        {isCalendarOpen && (
                          ...
                          >
                            <div className="flex items-center justify-between mb-4">
                              ...
                              <span className="font-medium text-slate-800">2026年 6月</span>
                            ...
                            <div className="grid grid-cols-7 gap-1 text-center text-sm">
                              <div className="aspect-square flex items-center justify-center rounded-md text-slate-300">31</div>
                            ...
                            <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-center space-x-4 text-xs text-slate-500">
                              <div className="flex items-center"><div className="w-2.5 h-2.5 bg-orange-400 rounded-full mr-1.5"></div>已签到</div>
                              <div className="flex items-center"><div className="w-2.5 h-2.5 bg-blue-100 border border-blue-200 rounded-full mr-1.5"></div>今日</div>
                            </div>
                          </motion.div>
                        )}
                      </AnimatePresence>
                ...
                {activeTenantId === 'personal' && (
                    ...
                    <div className="p-6 flex items-center">
                      <div className="flex-1">
                        <div className="flex flex-row items-center space-x-2 pb-2">
                          <h3 className="tracking-tight text-sm font-medium text-slate-500">Token (词元) 数量</h3>
                          <Activity className="h-4 w-4 text-blue-500" />
                        </div>
                        <div className="flex items-baseline space-x-2 mt-2">
                         <div className="text-3xl font-bold text-slate-800">860,666</div>
                         <span className="text-sm font-bold text-slate-600">剩余</span>
                       </div>
                        <div className="text-xs mt-4 space-y-1">
                          <div className="text-slate-500">总计: 1,000,000</div>
                          <div className="text-slate-500 flex items-center gap-1">已使用: <span className="w-2.5 h-2.5 bg-[#cbd5e1] rounded-sm inline-block"></span> 139,334</div>
                        </div>
                      </div>
                      <div className="w-32 h-32 relative">
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie
                              data={[{name: '剩余', value: 860666}, {name: '已使用', value: 139334}]}
                              cx="50%"
                              cy="50%"
                              innerRadius={30}
                              outerRadius={50}
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none pb-0.5">
                          <span className="text-xs text-blue-600 font-bold">86%</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white border text-card-foreground shadow-sm rounded-xl overflow-hidden hover:shadow-md transition-shadow relative">
                    <div className="p-6 flex items-center">
                      <div className="flex-1">
                        <div className="flex flex-row items-center space-x-2 pb-2">
                          <h3 className="tracking-tight text-sm font-medium text-slate-500">实验时长</h3>
                          <Clock className="h-4 w-4 text-emerald-500" />
                        </div>
                        <div className="flex items-baseline space-x-2 mt-2">
                         <div className="text-3xl font-bold text-slate-800">100</div>
                         <span className="text-sm font-bold text-slate-600">分钟 剩余</span>
                       </div>
                        <div className="text-xs mt-4 space-y-1">
                          <div className="text-slate-500">总计: 120 分钟</div>
                          <div className="text-slate-500 flex items-center gap-1">已使用: <span className="w-2.5 h-2.5 bg-[#cbd5e1] rounded-sm inline-block"></span> 20 分钟</div>
                        </div>
                      </div>
                      <div className="w-32 h-32 relative">
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie
                              data={[{name: '剩余', value: 100}, {name: '已使用', value: 20}]}
                              cx="50%"
                              cy="50%"
                              innerRadius={30}
                              outerRadius={50}
                            ...
                            <RechartsTooltip formatter={(value: number) => value + ' 分钟'} />
                          </PieChart>
                        </ResponsiveContainer>
                        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none pb-0.5">
                          <span className="text-xs text-emerald-600 font-bold">83%</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white border text-card-foreground shadow-sm rounded-xl overflow-hidden hover:shadow-md transition-shadow relative">
                    <div className="p-6 flex items-center">
                      <div className="flex-1">
                        <div className="flex flex-row items-center space-x-2 pb-2">
                          <h3 className="tracking-tight text-sm font-medium text-slate-500">PPT 生成次数</h3>
                          <FileText className="h-4 w-4 text-purple-500" />
                        </div>
                        <div className="flex items-baseline space-x-2 mt-2">
                         <div className="text-3xl font-bold text-slate-800">80</div>
                         <span className="text-sm font-bold text-slate-600">次 剩余</span>
                       </div>
                        <div className="text-xs mt-4 space-y-1">
                          <div className="text-slate-500">总计: 200 次</div>
                          <div className="text-slate-500 flex items-center gap-1">已使用: <span className="w-2.5 h-2.5 bg-[#cbd5e1] rounded-sm inline-block"></span> 120 次</div>
                        </div>
                      </div>
                      <div className="w-32 h-32 relative">
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie
                              data={[{name: '剩余', value: 80}, {name: '已使用', value: 120}]}
                              cx="50%"
                              cy="50%"
                              innerRadius={30}
                              outerRadius={50}
                            ...
                            <RechartsTooltip formatter={(value: number) => value + ' 次'} />
                          </PieChart>
                        </ResponsiveContainer>
                        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none pb-0.5">
                          <span className="text-xs text-purple-600 font-bold">40%</span>
                        </div>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="bg-white border text-card-foreground shadow-sm rounded-xl overflow-hidden">
                      <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex flex-wrap gap-3 items-center">
                        <div className="relative">
                          ...
                          <input 
                            type="text" 
                            placeholder="按时间搜索 (2026-06)" 
                            value={usageSearchQuery}
                            onChange={(e) => setUsageSearchQuery(e.target.value)}
                            className="pl-9 pr-3 py-1.5 bg-white border border-slate-200 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 w-56"
                          />
                        </div>
                        <div className="relative">
                          <Search className="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" />
                        ...
                        <div className="flex items-center space-x-1 ml-2 bg-slate-100/80 p-1 rounded-lg border border-slate-200">
                          {(['all', 'token', 'duration', 'ppt'] as const).map(type => (
                            <label key={type} className={`px-4 py-1.5 text-sm rounded-md cursor-pointer transition-colors ${usageTypeFilter === type ? 'bg-white shadow-sm text-blue-600 font-medium' : 'text-slate-600 hover:text-slate-800'}`}>
                              <input 
                                type="radio" 
                                name="usageType" 
                                value={type} 
                                ...
                                className="hidden"
                              />
                              {type === 'all' ? '全部消耗' : type === 'token' ? 'Token' : type === 'duration' ? '时长' : 'PPT'}
                            </label>
                          ))}
                        </div>
                        <select
                          value={usageSortOrder}
                          onChange={(e) => setUsageSortOrder(e.target.value as any)}
                        ...
                        >
                          ...
                          <option value="desc">消耗量从高到低</option>
                          <option value="asc">消耗量从低到高</option>
                        </select>
                      </div>
                        ...
                        <table className="w-full text-sm text-left">
                          <thead className="text-xs text-slate-500 bg-slate-50/80 border-b border-slate-200">
                            <tr>
                              ...
                              <th className="px-6 py-4 font-medium">使用账号</th>
                              <th className="px-6 py-4 font-medium">计费项（环境）</th>
                          ...
                          <tbody className="divide-y divide-slate-100">
                            {getFilteredAndSortedRecords().map((record) => (
                              <tr key={record.id} className="hover:bg-slate-50/50 transition-colors">
                                <td className="px-6 py-4 text-slate-600">{record.id}</td>
                                <td className="px-6 py-4 text-slate-600 whitespace-nowrap">{record.startTime}</td>
                                <td className="px-6 py-4 text-slate-600">{record.account}</td>
                                <td className="px-6 py-4 text-slate-800 break-words">{record.item}</td>
                                <td className="px-6 py-4 text-slate-600 whitespace-nowrap">
                                  {record.type === 'token' ? (
                                    <span className="text-blue-600 font-medium">{record.amount.toLocaleString()} Token(s)</span>
                                  ) : record.type === 'duration' ? (
                                    <span className="text-emerald-600 font-medium">{record.amount} 分钟</span>
                                  ) : (
                                    <span className="text-purple-600 font-medium">{record.amount} 次</span>
                                  )}
                                </td>
                              </tr>
                            ))}
                            {getFilteredAndSortedRecords().length === 0 && (
                              <tr>
                                <td colSpan={5} className="px-6 py-8 text-center text-slate-400">没有找到匹配的记录</td>
                              </tr>
                            )}
                          </tbody>
                        </table>
           ...
           {activeMenu === 'advanced_security' && (
             <div className="p-8">
               <h1 className="text-2xl font-bold text-slate-800 mb-6">安全设置</h1>
               <div className="max-w-4xl divide-y divide-slate-100">
                 <div className="py-6 flex items-center justify-between">
                   <div>
                     <h3 className="text-base font-medium text-slate-800 mb-1">绑定手机</h3>
                 ...
                 <div className="py-6 flex items-center justify-between">
                   <div>
                     <h3 className="text-base font-medium text-slate-800 mb-1">绑定微信</h3>
                     {isWechatBound ? (
                       <p className="text-sm text-slate-500">已绑定 <span className="text-green-600">UUSIMA_User</span></p>
                     ) : (
                       <p className="text-sm text-slate-500">未绑定</p>
                     )}
                   </div>
                   {isWechatBound ? (
                     <button onClick={() => setIsWechatBound(false)} className="text-sm font-medium text-slate-400 hover:text-slate-600 transition-colors group relative">
                       解除绑定
                     </button>
                   ) : (
                     <button onClick={() => setIsWechatModalOpen(true)} className="text-sm font-medium text-blue-600 hover:text-blue-700 transition-colors">
                       去绑定
                     </button>
                   )}
                 </div>
                 ...
                 <div className="py-6 flex items-center justify-between">
                   <div>
                     <h3 className="text-base font-medium text-slate-800 mb-1">登录密码</h3>
         ...
         {isPhoneVerifyModalOpen && (
           <>
             ...
             <div className="fixed inset-0 z-[101] flex items-center justify-center p-4 pointer-events-none">
               ...
               >
                 <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                   <h2 className="text-lg font-semibold text-slate-900">手机号码认证</h2>
                   <button 
                     onClick={() => setIsPhoneVerifyModalOpen(false)}
                     className="p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-full transition-colors"
                   >
                ...
                >
                  ...
                  <div className="p-6 overflow-y-auto w-full text-sm">
                     <div className="grid grid-cols-[100px_1fr] gap-y-4 gap-x-4">
                        {adminApprovalDetailModal.details.map((detail: any, idx: number) => (
                           <React.Fragment key={idx}>
                             <div className="text-slate-500 text-right">{detail.label}</div>
                             <div className="font-medium text-slate-800">
                               {detail.isAttachment ? (
                                 <span className="text-blue-600 flex items-center cursor-pointer hover:underline text-sm"><BadgeCheck className="w-4 h-4 mr-1"/> {detail.value}</span>
                               ) : (
                                 detail.value
                               )}
                             </div>
                     ...
                     {isRejectingEntity && (
                       <div className="mt-6 pt-4 border-t border-slate-100 animate-in fade-in slide-in-from-top-2">
                         <label className="block font-medium text-slate-700 mb-2">驳回原因 <span className="text-red-500">*</span></label>
                         <textarea 
                         ...
                         <div className="flex justify-end space-x-2 mt-3">
                           <button onClick={() => {setIsRejectingEntity(false); setRejectReason('');}} className="px-4 py-2 text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-lg text-sm font-medium transition-colors">取消</button>
                           <button 
                             onClick={() => {
                               if(!rejectReason) {
                                 alert('请输入驳回原因'); return;
                               }
                               setAdminApprovals(prev => prev.map(a => a.id === adminApprovalDetailModal.id ? {...a, status: 'rejected', rejectReason} : a));
                               setAdminApprovalDetailModal(null);
                               setIsRejectingEntity(false);
                               setRejectReason('');
                             }} 
                  ...
                  {!isRejectingEntity && (
                    <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end space-x-3 shrink-0">
                       <button onClick={() => setIsRejectingEntity(true)} className="px-5 py-2 font-medium text-red-600 bg-white border border-red-200 rounded-lg hover:bg-red-50 transition-colors shadow-sm">
                         驳 回
                       </button>
                       <button 
                         onClick={() => {
                            setAdminApprovals(prev => prev.map(a => a.id === adminApprovalDetailModal.id ? {...a, status: 'approved'} : a));
                            setAdminApprovalDetailModal(null);
                         }} 
              ...
              <div className="fixed inset-0 z-[101] flex items-center justify-center p-4 pointer-events-none">
                ...
                >
                  <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100">
                    ...
                    <button onClick={() => setIsWechatModalOpen(false)} className="text-gray-400 hover:text-gray-500 transition-colors">
                      <X className="w-5 h-5" />
                    </button>
                  </div>
                  <div className="p-8 flex flex-col items-center">
                    ...
                    <p className="text-slate-600 text-sm text-center">打开微信扫一扫，关注并绑定</p>
                  </div>
              ...
              <div className="fixed inset-0 z-[101] flex items-center justify-center p-4 pointer-events-none">
                ...
                >
                  <div className="p-6 pb-0">
                    ...
                    <h3 className="text-lg font-bold text-slate-800 mb-2">确认退出组织？</h3>
                    <p className="text-sm text-slate-600 mb-4 whitespace-normal break-words">
                      确定要退出组织 <span className="font-semibold text-slate-800">“{orgToExit}”</span> 吗？
                    </p>
                  ...
                  <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end space-x-3">
                    ...
                    >
                      {exitOrgCountdown > 0 ? `确定退出 (${exitOrgCountdown}s)` : '确定退出'}
                    </button>
                  </div>
                </motion.div>
              </div>
              ...
              <div className="fixed inset-0 z-[101] flex items-center justify-center p-4 pointer-events-none">
                ...
                >
                  ...
                  <div className="flex-none p-6 pb-4">
                    <h2 className="text-xl font-bold text-slate-800 mb-2">组织申请记录</h2>
                    <p className="text-sm text-slate-500">查看和管理我收到的组织加入邀请记录</p>
                  </div>
                  ...
                  <div className="flex-1 overflow-auto px-6 pb-6">
                    <table className="w-full text-left text-sm whitespace-nowrap">
                      <thead className="bg-slate-50 sticky top-0 z-10 before:content-[''] before:absolute before:inset-0 before:border-b before:border-slate-200 before:-z-10">
                        <tr>
                          <th className="px-4 py-3.5 font-medium text-slate-600">组织机构</th>
                          <th className="px-4 py-3.5 font-medium text-slate-600">申请人</th>
                          <th className="px-4 py-3.5 font-medium text-slate-600">手机号</th>
                          <th className="px-4 py-3.5 font-medium text-slate-600">申请时间</th>
                          <th className="px-4 py-3.5 font-medium text-slate-600">处理时间</th>
                          <th className="px-4 py-3.5 font-medium text-slate-600">状态</th>
                          <th className="px-4 py-3.5 font-medium text-slate-600">邀请链接</th>
                          <th className="px-4 py-3.5 font-medium text-slate-600">分配角色</th>
                          <th className="px-4 py-3.5 font-medium text-slate-600">驳回原因</th>
                          <th className="px-4 py-3.5 font-medium text-slate-600 bg-slate-50 sticky right-0">操作</th>
                        </tr>
                      </thead>
                      <tbody>
                        {/* Empty state */}
                        <tr>
                          <td colSpan={10} className="px-4 py-32 text-center text-slate-400">
                            暂无内容
                          </td>
                        </tr>
                      </tbody>
        ...
        <AnimatePresence>
          {isSignToastOpen && (
            ...
            >
              ...
              <div>
                <div className="text-[15px] font-medium text-slate-800">签到成功！</div>
                <div className="text-sm text-slate-500 mt-0.5">奖励：1小时实验时长 + 1万Token</div>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
