...
# 1. Remove the standalone tenant button
tenant_btn_pattern = r"""\s*<button\s*onClick=\{\(\) => setIsTenantOpen\(true\)\}\s*className="flex items-center text-sm mr-4 border border-slate-200 rounded-md px-3 py-1\.5 hover:bg-slate-50 transition-colors shadow-sm"\s*>\s*<span className="font-medium text-slate-700">\s*\{activeTenant\.name\}\s*</span>\s*<ArrowLeftRight className="w-3\.5 h-3\.5 ml-2 text-slate-400" />\s*</button>"""
content = re.sub(tenant_btn_pattern, "", content)

# 2. Update the dropdown menu content
